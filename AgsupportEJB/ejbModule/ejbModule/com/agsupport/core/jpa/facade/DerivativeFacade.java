...
public class DerivativeFacade {
	...
	public List<Derivative> getDerivativeList() {
		try {
			TypedQuery<Derivative> query = em.createQuery("SELECT d FROM Derivative d ORDER BY d.name",
					Derivative.class);
	...
	public Derivative getDerivativeByName(String name) {
		try {
			TypedQuery<Derivative> query = em.createQuery(
					"SELECT d FROM Derivative d WHERE d.name = :name", Derivative.class);
	...
	/**
	 * Dodanie wartości do instrumentu pochodnego. Dodanie wartości zachodzi przy pewnych
	 * warunkach. Warunki: Wartości instrumentu pochodnego dotyczące tego samego
	 * instrumentu pochodnego, wygaśnięcia w tym samym miesiącu i roku, nie
	 * posiadają tych samych wartości i występują w bazie w bezpośrednim
	 * sąsiedzwie pod względem daty dodania do bazy
	 * 
	 * @param derivativeId
	 *            id instrumentu pochodnego
	...
	public boolean addDerivativeValue(long derivativeId,
		...
		try {
			...
			query.setParameter("derivativeId", derivativeId).setParameter(
					"expiredDate", derivativeValue.getExpiredDate(),
